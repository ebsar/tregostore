...
struct TregoNativeRootView: View {
    ...
    @State private var showsLaunchPromo = false
    ...
    var body: some View {
        ZStack {
            ...
            if showsLaunchPromo {
                TregoLaunchPromoOverlay(
                    isPresented: $showsLaunchPromo,
                    launchAds: startupLaunchAds,
                    onShopNow: {
        ...
        .fullScreenCover(item: $store.selectedProduct) { product in
            NavigationView {
        ...
        .task {
            ...
            if !startupLaunchAds.isEmpty {
                try? await Task.sleep(nanoseconds: 180_000_000)
                withAnimation(.spring(response: 0.34, dampingFraction: 0.9)) {
                    showsLaunchPromo = true
                }
            }
        }
    ...
    private var startupLaunchAds: [TregoLaunchAd] {
        store.launchAds
            .filter { ad in
                (ad.isActive ?? true)
                && !TregoLaunchAdFormDraft.isSliderPlaceholderTitle(ad.title)
                && !(ad.title?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true)
                && !(ad.imagePath?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true)
            }
...
private struct TregoHomeAdsHeroCarousel: View {
    let ads: [TregoLaunchAd]
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            TabView(selection: $selectedIndex) {
                ForEach(Array(resolvedAds.enumerated()), id: \.element.id) { index, ad in
                    Button(action: onPrimaryAction) {
                        ZStack {
                            TregoRemoteImage(imagePath: ad.imagePath)
                                .scaledToFill()
            ...
            HStack(spacing: 7) {
                ForEach(Array(resolvedAds.indices), id: \.self) { index in
                    Capsule()
                        .fill(index == selectedIndex ? TregoNativeTheme.accent : Color.secondary.opacity(0.28))
    ...
    private var resolvedAds: [TregoLaunchAd] {
        let slides = ads.filter { launchAd in
            let imagePath = launchAd.imagePath?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            return !imagePath.isEmpty && TregoLaunchAdFormDraft.isSliderDisplayAd(launchAd)
        }
        return slides
    }
}

private struct TregoHomeCategoryRailButton: View {
    let title: String
    ...
    var body: some View {
        VStack(spacing: 8) {
            Image(systemName: systemImage)
                ...
                .foregroundStyle(isSelected ? .white : TregoNativeTheme.accentStrong)
            ...
            Text(title)
                ...
                .foregroundStyle(isSelected ? TregoNativeTheme.primaryText : TregoNativeTheme.secondaryText)
                .lineLimit(1)
        }
        .frame(width: 78)
        .padding(.vertical, 8)
...
private struct TregoHomeAdsHeroSkeleton: View {
    var body: some View {
        RoundedRectangle(cornerRadius: 32, style: .continuous)
            .fill(TregoNativeTheme.cardFill.opacity(0.72))
            .frame(height: 242)
            .redacted(reason: .placeholder)
    }
}

private struct TregoHomeCategoryRailSkeleton: View {
    var body: some View {
        HStack(spacing: 12) {
...
private struct TregoHomeScreen: View {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    if store.homeLoading && store.homeProducts.isEmpty {
                        TregoHomeAdsHeroSkeleton()
                        TregoHomeCategoryRailSkeleton()
                        ...
                        if !activeLaunchAds.isEmpty {
                            TregoHomeAdsHeroCarousel(
                                ads: activeLaunchAds,
                                onPrimaryAction: {
                                    ...
                                    HStack(spacing: 10) {
                                        ForEach(homeCategoryOptions) { option in
                                            Button {
                                                ...
                                                TregoHomeCategoryRailButton(
                                                    title: option.title,
                                                    systemImage: homeCategoryIcon(for: option.key),
                                                    isSelected: selectedHomeCategoryKey == option.key
                                                )
                                            }
                                            .buttonStyle(.plain)
                                        }
    ...
    private var activeLaunchAds: [TregoLaunchAd] {
        let activeAds = store.launchAds
            .filter { $0.isActive ?? true }
            .filter { !($0.imagePath?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true) }
            .sorted { ($0.sortOrder ?? 0) < ($1.sortOrder ?? 0) }

        let preferredSliderAds = activeAds.filter { TregoLaunchAdFormDraft.isSliderDisplayAd($0) }
        return preferredSliderAds.isEmpty ? activeAds : preferredSliderAds
    }
    ...
    private var hasAnyHomeContent: Bool {
        !activeLaunchAds.isEmpty || !recommendedGridProducts.isEmpty
    }
    ...
    private func homeCategoryIcon(for key: String) -> String {
        switch key {
        case "fashion":
            return "tshirt.fill"
        case "tech":
...
private struct TregoSearchScreen: View {
    ...
    @State private var pickerSource: TregoImagePickerSource?
    @State private var alertState: TregoSearchBarAlert?
    @State private var selectedSortOption: TregoSearchSortOption = .relevance
            ...
            ScrollView(.vertical, showsIndicators: false) {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    if store.searchLoading {
                        ...
                        LazyVGrid(columns: grid, spacing: 14) {
                            ForEach(sortedSearchResults) { product in
                                TregoProductCard(
                                    product: product,
                                    isWishlisted: store.isWishlisted(productId: product.id),
                                    onTap: { store.selectedProduct = product },
                                ...
                                .onAppear {
                                    if product.id == sortedSearchResults.last?.id {
    ...
    private var sortedSearchResults: [TregoProduct] {
        switch selectedSortOption {
        case .relevance:
            return store.searchResults
        case .latest:
            return store.searchResults.sorted { String($0.createdAt ?? "") > String($1.createdAt ?? "") }
        case .priceLowHigh:
            return store.searchResults.sorted { ($0.price ?? .greatestFiniteMagnitude) < ($1.price ?? .greatestFiniteMagnitude) }
        case .priceHighLow:
            return store.searchResults.sorted { ($0.price ?? 0) > ($1.price ?? 0) }
        case .topRated:
            return store.searchResults.sorted { ($0.averageRating ?? 0) > ($1.averageRating ?? 0) }
        }
    }
...
private struct TregoSearchScreenNavigationChrome: ViewModifier {
    ...
    let onSubmit: () -> Void
    let onSortSelect: (TregoSearchSortOption) -> Void
            ...
            content
                ...
                .toolbar {
                    ToolbarItem(placement: .topBarLeading) {
                        Menu {
                            ForEach(TregoSearchSortOption.allCases) { option in
                                Button(option.title) {
                                    onSortSelect(option)
                                }
                            }
...
private enum TregoSearchSortOption: String, CaseIterable, Identifiable {
    case relevance
    case latest
    case priceLowHigh
    case priceHighLow
    ...
    var body: some View {
        NavigationView {
                    ...
                    Group {
                        if store.isResolvingSession || store.cartLoading {
                            ScrollView(.vertical, showsIndicators: false) {
                                VStack(alignment: .leading, spacing: 18) {
                                    ForEach(0..<2, id: \.self) { _ in
                                        TregoCartBusinessGroupSkeleton()
                                    }
                                }
                                .padding(.horizontal, 16)
                                .padding(.top, 8)
            ...
            .safeAreaInset(edge: .bottom) {
                if store.user != nil && (store.cartLoading || !store.cart.isEmpty) {
                            ...
                            HStack(spacing: 10) {
                                ...
                                HStack(spacing: 10) {
                                    if showsShortcutSearch {
                                        ...
                                        .padding(.vertical, 12)
                                        .frame(width: min(UIScreen.main.bounds.width - 104, 290))
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 14) {
                ...
                TregoSettingsSectionCard(title: "Pamja dhe gjuha") {
                    VStack(spacing: 0) {
                        TregoSettingsPickerAccountRow(
                            title: "Theme",
                            value: label(for: store.appSettings.theme, options: themeOptions),
                            systemImage: "circle.lefthalf.filled",
                            helper: "Pamja e aplikacionit",
                            options: themeOptions
                        ) { store.updateAppTheme($0) }
                        Divider().overlay(Color.white.opacity(0.08))
                        TregoSettingsPickerAccountRow(
                            title: "Gjuha",
                            value: label(for: store.appSettings.language, options: languageOptions),
                            systemImage: "globe",
                            helper: "Gjuha kryesore e app-it",
                            options: languageOptions
                        ) { store.updateAppLanguage($0) }
                        Divider().overlay(Color.white.opacity(0.08))
                        TregoSettingsPickerAccountRow(
                            title: "Valuta",
                            value: label(for: store.appSettings.currency, options: currencyOptions),
                            systemImage: "eurosign.circle.fill",
                            helper: "Valuta e preferuar per shfaqje",
                            options: currencyOptions
                        ) { store.updateAppCurrency($0) }
                    }
                }

                TregoSettingsSectionCard(title: "Njoftimet") {
                    VStack(spacing: 0) {
                        TregoSettingsPickerAccountRow(
                            title: "Njoftimet",
                            value: label(for: store.appSettings.notificationMode, options: notificationOptions),
                            systemImage: "bell.badge.fill",
                            helper: "Cfare lloji njoftimesh deshironi",
                        ...
                        Button {
                            Task {
                                ...
                                store.globalMessage = response.message ?? (response.ok == true ? "Push notifications u aktivizuan." : "Njoftimet nuk u aktivizuan.")
                            }
                        } label: {
                            TregoActionAccountRow(
                                title: "Push notifications",
                                value: "",
                                systemImage: "app.badge.fill",
                                helper: "Lock screen alerts per porosi dhe mesazhe"
                            )
                        }
                        .buttonStyle(.plain)
#if DEBUG
                        ...
                        Button {
                            Task {
                                ...
                                store.globalMessage = response.message ?? (response.ok == true ? "Njoftimi lokal u planifikua." : "Njoftimi lokal nuk u planifikua.")
                            }
                        } label: {
                            TregoActionAccountRow(
                                ...
                                helper: "Simulon nje push me njoftim lokal ne kete build"
                            )
                        }
                        .buttonStyle(.plain)
#endif
                ...
                TregoSettingsSectionCard(title: "Privatesia") {
                    VStack(spacing: 0) {
                        TregoSettingsPickerAccountRow(
                            title: "Privatesia",
                            value: label(for: store.appSettings.privacyMode, options: privacyOptions),
                            systemImage: "lock.shield.fill",
                            helper: "Kontrolli i rekomandimeve dhe tracking-ut",
                        ...
                        Button {
                            ...
                            TregoActionAccountRow(
                                ...
                                value: trackingConsentSubtitle,
                                systemImage: "hand.raised.fill",
                                helper: "Menaxho personalizimin e produkteve"
                            )
                        }
                        .buttonStyle(.plain)
                    }
...
private struct TregoPersonalDataScreen: View {
    ...
    @State private var showsContactEditorSheet = false
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 18) {
                if let user = store.user {
                    ...
                    TregoSettingsSectionCard(title: "Kontakt") {
                        VStack(alignment: .leading, spacing: 12) {
                            HStack(alignment: .top, spacing: 12) {
                                ...
                                Button {
                                    Task { await requestSecureContactEditing() }
                                } label: {
                                    Label("Edito", systemImage: "pencil")
                                }
                    ...
                    TregoSettingsSectionCard(title: "Detajet personale") {
                        VStack(spacing: 0) {
                            TregoEditableAccountTextRow(
                                title: "Emri",
                                text: $firstName,
                                placeholder: "Shkruaj emrin",
                                systemImage: "person.fill",
                                helper: "Emri i perdorur ne profilin tend",
                            ...
                            TregoEditableAccountTextRow(
                                ...
                                helper: "Mbiemri i lidhur me llogarine tende",
                                textContentType: .familyName,
                                disableAutocorrection: false
                            )
                            Divider()
                                .overlay(Color.white.opacity(0.08))
                            TregoEditableBirthDateRow(
                                birthDate: $birthDate
                            )
                            Divider()
                                .overlay(Color.white.opacity(0.08))
                            TregoEditableGenderRow(
                                gender: $gender
                            )
                        }
        ...
        .sheet(isPresented: $showsContactEditorSheet) {
            NavigationView {
                TregoContactEditorSheet(
                    email: $contactEmail,
                    phoneNumber: $contactPhoneNumber,
                    onCancel: {
                        hydrateFromCurrentUser()
                        showsContactEditorSheet = false
                    },
                    onSave: {
                        showsContactEditorSheet = false
                    }
    ...
    private func saveProfile() async {
        ...
        let (response, _) = await store.api.updateProfile(
            ...
            phoneNumber: contactPhoneNumber.trimmingCharacters(in: .whitespacesAndNewlines)
        )
    ...
    private func requestSecureContactEditing() async {
        let context = LAContext()
        context.localizedCancelTitle = "Anulo"

        var authError: NSError?
        let reason = "Perdor Face ID per te ndryshuar email-in ose numrin e telefonit."

        guard context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &authError) else {
            await MainActor.run {
                store.globalMessage = "Face ID nuk eshte i disponueshem ne kete pajisje."
            }
            return
        }

        do {
            let granted = try await context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason)
            guard granted else { return }
            await MainActor.run {
                showsContactEditorSheet = true
            }
        } catch {
            await MainActor.run {
                store.globalMessage = "Autentikimi me Face ID nuk u krye."
            }
        }
    }
        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                if store.user != nil {
                    ...
                    TregoSettingsSectionCard(title: "Siguria") {
                        VStack(spacing: 0) {
                            TregoEditableSecureAccountRow(
                                title: "Fjalëkalimi aktual",
                                text: $currentPassword,
                                placeholder: "Shkruaj fjalekalimin aktual",
                                systemImage: "lock.fill",
                                helper: "Per verifikimin e llogarise",
                            ...
                            TregoEditableSecureAccountRow(
                                ...
                                helper: "Perdor nje fjalekalim te forte",
                                textContentType: .newPassword,
                                submitLabel: .next
                            )
                            Divider().overlay(Color.white.opacity(0.08))
                            TregoEditableSecureAccountRow(
                                title: "Konfirmo fjalëkalimin",
                                text: $confirmPassword,
                                placeholder: "Perserite fjalekalimin e ri",
                                systemImage: "checkmark.shield.fill",
                                helper: "Duhet te perputhet me fjalekalimin e ri",
                                textContentType: .newPassword,
                                submitLabel: .done
                            )
                        }
...
private struct TregoContactEditorSheet: View {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                TregoNoticeCard(
                    title: "Perditeso kontaktin",
                    subtitle: "Email-i dhe telefoni jane zhbllokuar me Face ID para editimit."
                )
        ...
        .toolbar {
            ToolbarItem(placement: .cancellationAction) {
                Button("Anulo", action: onCancel)
            }
            ToolbarItem(placement: .confirmationAction) {
                Button("Ruaj", action: onSave)
            }
        }
    }
}
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                if store.user != nil {
                    if let savedAddress {
                        TregoSettingsSectionCard(title: "Adresa default") {
                            VStack(spacing: 0) {
                                TregoAccountDetailRow(title: "Adresa", value: savedAddress.addressLine ?? "", systemImage: "mappin.and.ellipse", helper: "Adresa aktuale e ruajtur")
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoAccountDetailRow(title: "Qyteti", value: savedAddress.city ?? "", systemImage: "building.2.fill", helper: "Qyteti i dergeses")
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoAccountDetailRow(title: "Shteti", value: savedAddress.country ?? "", systemImage: "globe.europe.africa.fill", helper: "Shteti i ruajtur")
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoAccountDetailRow(title: "Zip code", value: savedAddress.zipCode ?? "", systemImage: "number.square.fill", helper: "Kodi postar")
                            }
                        }
                    }
                    ...
                    if isLoading && savedAddress == nil {
                        ...
                        TregoSettingsSectionCard(title: "Perditeso adresen") {
                            VStack(spacing: 0) {
                                Button {
                                    locationHelper.requestCurrentAddress()
                                } label: {
                                    TregoActionAccountRow(
                                        title: "Perdor lokacionin aktual",
                                        value: "",
                                        systemImage: locationHelper.isLoading ? "location.magnifyingglass" : "location.fill",
                                        helper: locationHelper.isLoading ? "Po marr adresen nga lokacioni..." : "Mbush automatikisht adresen me vendndodhjen tende"
                                    )
                                }
                                .buttonStyle(.plain)
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoEditableAccountTextRow(title: "Adresa", text: $addressLine, placeholder: "Rruga, numri, hyrja", systemImage: "mappin.and.ellipse", helper: "Adresa e vendbanimit", textContentType: .fullStreetAddress, disableAutocorrection: false)
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoEditableAccountTextRow(title: "Qyteti", text: $city, placeholder: "Shkruaj qytetin", systemImage: "building.2.fill", helper: "Qyteti ku banon", textContentType: .addressCity, disableAutocorrection: false)
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoEditableAccountTextRow(title: "Shteti", text: $country, placeholder: "Shkruaj shtetin", systemImage: "globe.europe.africa.fill", helper: "Shteti i dergeses", textContentType: .countryName, disableAutocorrection: false)
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoEditableAccountTextRow(title: "Zip code", text: $zipCode, placeholder: "Shkruaj zip code", systemImage: "number.square.fill", helper: "Kodi postar", textContentType: .postalCode)
                                Divider().overlay(Color.white.opacity(0.08))
                                TregoEditableAccountTextRow(title: "Telefoni", text: $phoneNumber, placeholder: "+383 44 123 456", systemImage: "phone.fill", helper: "Numri per kontakt me korrierin", textContentType: .telephoneNumber, keyboardType: .phonePad, textInputAutocapitalization: .never)
                            }
                        }
...
private struct TregoNotificationsScreen: View {
    ...
    @AppStorage("trego.notifications.messages") private var allowsMessageNotifications = true
    @AppStorage("trego.notifications.orders") private var allowsOrderNotifications = true
    @AppStorage("trego.notifications.promotions") private var allowsPromotionNotifications = true
    ...
    var body: some View {
            ...
            if let notification = selectedNotification {
                ...
                TregoNotificationDetailSheet(
                    ...
                    onOpen: {
                        ...
                        Task { await store.openNotificationItem(openedNotification) }
                    }
                )
                .frame(maxWidth: 360)
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel("Rregullimet e njoftimeve")
            }
        }
        .task {
        ...
        .sheet(isPresented: $showsNotificationSettings) {
            NavigationView {
                TregoNotificationSettingsScreen(
                    allowsMessageNotifications: $allowsMessageNotifications,
                    allowsOrderNotifications: $allowsOrderNotifications,
                    allowsPromotionNotifications: $allowsPromotionNotifications
                )
            }
            .navigationViewStyle(.stack)
        }
    ...
    private func loadNotifications() async {
        ...
        let filtered = payload.notifications.filter(shouldDisplayNotification)
        notifications = filtered
        unreadCount = filtered.filter { $0.isRead != true }.count
        message = ""
        messageTone = nil
    ...
    private func shouldDisplayNotification(_ notification: TregoNotificationItem) -> Bool {
        switch (notification.type ?? "").trimmingCharacters(in: .whitespacesAndNewlines).lowercased() {
        case "message":
            return allowsMessageNotifications
        case "order", "return", "verification":
            return allowsOrderNotifications
        case "promotion":
            return allowsPromotionNotifications
        default:
            return true
        }
    }
...
private struct TregoNotificationSettingsScreen: View {
    @Environment(\.dismiss) private var dismiss
    @Binding var allowsMessageNotifications: Bool
    @Binding var allowsOrderNotifications: Bool
    @Binding var allowsPromotionNotifications: Bool

    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                TregoNoticeCard(
                    title: "Rregullimet e njoftimeve",
                    subtitle: "Zgjidh cilat njoftime do te shfaqen ne qendren e njoftimeve te app-it."
                )

                TregoNotificationPreferenceRow(
                    title: "Mesazhe nga pronari",
                    subtitle: "Bisedat dhe pergjigjet nga bizneset.",
                    symbolName: "message.fill",
                    isEnabled: $allowsMessageNotifications
                )

                TregoNotificationPreferenceRow(
                    title: "Statusi i porosise",
                    subtitle: "Konfirmime, dergesa, kthime dhe verifikime.",
                    symbolName: "shippingbox.fill",
                    isEnabled: $allowsOrderNotifications
                )

                TregoNotificationPreferenceRow(
                    title: "Promocione",
                    subtitle: "Oferta, zbritje dhe lajmerime promocionale.",
                    symbolName: "sparkles",
                    isEnabled: $allowsPromotionNotifications
                )
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 18)
        ...
        .background(TregoNativeTheme.systemBackground.ignoresSafeArea())
        .navigationTitle("Njoftime")
        .navigationBarTitleDisplayMode(.inline)
        .toolbar {
            ToolbarItem(placement: .cancellationAction) {
                Button("Mbyll") { dismiss() }
            }
        }
    }
}

private struct TregoNotificationPreferenceRow: View {
    let title: String
    let subtitle: String
...
private struct TregoBusinessHubScreen: View {
    ...
    @State private var showsAllManagedProducts = false
                ...
                if store.user?.role != "business" {
                    ...
                    if store.businessWorkspaceLoading && store.businessProfile == nil {
                        ...
                        if !store.businessProducts.isEmpty {
                            VStack(spacing: 12) {
                                ForEach(visibleBusinessProducts) { product in
                            ...
                            if store.businessProducts.count > 3 {
                                Button(showsAllManagedProducts ? "Show less" : "Show more") {
                                    withAnimation(.spring(response: 0.28, dampingFraction: 0.88)) {
                                        showsAllManagedProducts.toggle()
                                    }
                                }
                                .buttonStyle(TregoMiniButtonStyle(tint: TregoNativeTheme.softAccent))
                            }
    ...
    private var visibleBusinessProducts: [TregoProduct] {
        showsAllManagedProducts ? store.businessProducts : Array(store.businessProducts.prefix(3))
    }
}
...
private struct TregoAdminControlScreen: View {
    ...
    @State private var selectedUserCategory: TregoAdminUserCategory = .admin
                ...
                if store.user?.role != "admin" {
                    ...
                    if store.adminWorkspaceLoading && store.adminUsers.isEmpty && store.adminBusinesses.isEmpty {
                        ...
                        if !store.adminUsers.isEmpty {
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 10) {
                                    ForEach(TregoAdminUserCategory.allCases) { category in
                                        Button {
                                            selectedUserCategory = category
                                        } label: {
                                            Text(category.title)
                                                .font(.system(size: 13, weight: .bold, design: .rounded))
                                                .foregroundStyle(selectedUserCategory == category ? Color.white : TregoNativeTheme.primaryText)
                                                .padding(.horizontal, 14)
                                                .padding(.vertical, 10)
                                                .background {
                                                    if selectedUserCategory == category {
                                                        Capsule().fill(TregoNativeTheme.accent)
                                                    } else {
                                                        Capsule().fill(TregoNativeTheme.cardFill)
                                                    }
                                                }
                                        }
                                        .buttonStyle(.plain)
    ...
    private var filteredAdminUsers: [TregoAdminUser] {
        store.adminUsers.filter { user in
            switch selectedUserCategory {
            case .admin:
                return normalized(user.role) == "admin"
            case .business:
                return normalized(user.role) == "business"
            case .client:
                let role = normalized(user.role)
                return role == "client" || role == "user" || role.isEmpty
            }
        }
    }
...
private struct TregoOrderStatusFilter: Identifiable {
    let id: String
    let label: String
    let statuses: Set<String>
    let includesReturnActivity: Bool
}

private let tregoOrderStatusFilters: [TregoOrderStatusFilter] = [
    .init(id: "all", label: "All", statuses: [], includesReturnActivity: false),
    .init(id: "waiting_shipping", label: "Waiting shipping", statuses: ["pending_confirmation", "confirmed", "packed", "partially_confirmed"], includesReturnActivity: false),
    .init(id: "shipped", label: "Shipped", statuses: ["shipped"], includesReturnActivity: false),
    .init(id: "for_review", label: "For review", statuses: ["delivered"], includesReturnActivity: false),
    .init(id: "return_refund", label: "Return / Refund", statuses: ["returned", "refunded"], includesReturnActivity: true),
    .init(id: "cancelled", label: "Cancelled", statuses: ["cancelled", "canceled", "failed"], includesReturnActivity: false),
]

private struct TregoOrderStatusFilterRail: View {
    let filters: [TregoOrderStatusFilter]
    @Binding var selectedFilterId: String
    let orders: [TregoOrderItem]

    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 10) {
                ForEach(filters) { filter in
                    TregoOrderStatusFilterChip(
                        filter: filter,
                        count: orderCount(for: filter),
    ...
    private func orderCount(for filter: TregoOrderStatusFilter) -> Int {
        orders.filter { tregoOrder($0, matches: filter) }.count
    }
}

private struct TregoOrderStatusFilterChip: View {
    let filter: TregoOrderStatusFilter
    let count: Int
    let isSelected: Bool
    let onSelect: () -> Void
    ...
    var body: some View {
        Button(action: onSelect) {
            ...
            .foregroundStyle(isSelected ? Color.white : TregoNativeTheme.primaryText)
            .padding(.horizontal, 12)
            .padding(.vertical, 10)
            .background(chipBackground, in: Capsule())
            .overlay {
                Capsule()
                    .strokeBorder(TregoNativeTheme.accent.opacity(isSelected ? 0 : 0.2), lineWidth: 1)
            }
        }
        .buttonStyle(.plain)
    }
    ...
    private var chipBackground: AnyShapeStyle {
        isSelected ? AnyShapeStyle(TregoNativeTheme.accent) : TregoNativeTheme.cardFill
    }

    private var countBackground: Color {
        isSelected ? Color.white.opacity(0.24) : TregoNativeTheme.accent.opacity(0.12)
    }
}

private func tregoNormalizedOrderStatus(_ order: TregoOrderItem) -> String {
    (order.fulfillmentStatus ?? order.status ?? "pending_confirmation")
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
}

private func tregoOrderHasReturnActivity(_ order: TregoOrderItem) -> Bool {
    let returnStatus = (order.returnRequestStatus ?? "")
        .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    return order.items?.contains { item in
        !(item.returnRequestStatus ?? "").trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    } == true
}

private func tregoReturnableOrderItemId(_ order: TregoOrderItem) -> Int? {
    if let item = order.items?.first(where: { item in
        let itemStatus = (item.fulfillmentStatus ?? item.status ?? tregoNormalizedOrderStatus(order))
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        let returnStatus = (item.returnRequestStatus ?? "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        return (itemStatus == "delivered" || itemStatus == "returned")
            && !["requested", "approved", "received"].contains(returnStatus)
    }) {
        return item.id
    }

    let orderReturnStatus = (order.returnRequestStatus ?? "")
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    if ["delivered", "returned"].contains(tregoNormalizedOrderStatus(order))
        && !["requested", "approved", "received"].contains(orderReturnStatus) {
        return order.id
    }
...
private func tregoOrder(_ order: TregoOrderItem, matches filter: TregoOrderStatusFilter) -> Bool {
    if filter.id == "all" {
        return true
    }

    if filter.includesReturnActivity && tregoOrderHasReturnActivity(order) {
        return true
    }

    return filter.statuses.contains(tregoNormalizedOrderStatus(order))
}
...
private struct TregoManagedOrdersScreen: View {
        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                if orders.isEmpty {
                    ...
                    TregoOrderStatusFilterRail(
                        filters: tregoOrderStatusFilters,
                        selectedFilterId: $selectedStatusFilter,
                        orders: orders
                    )

                    if filteredOrders.isEmpty {
                        TregoEmptyStateView(
                            title: "Nuk ka porosi ne kete status",
                            subtitle: "Ndrysho filtrin per te pare waiting shipping, shipped, for review ose return/refund."
                        )
                    }
    ...
    private var filteredOrders: [TregoOrderItem] {
        guard let filter = tregoOrderStatusFilters.first(where: { $0.id == selectedStatusFilter }) else {
            return orders
        }
        return orders.filter { tregoOrder($0, matches: filter) }
    }
...
private struct TregoOrdersScreen: View {
            ...
            VStack(alignment: .leading, spacing: 14) {
                if store.ordersLoading {
                    ...
                    if filteredOrders.isEmpty {
                        TregoEmptyStateView(
                            title: "Nuk ka porosi ne kete status",
                            subtitle: "Provo Waiting shipping, Shipped, For review ose Return / Refund."
                        )
                    }
    ...
    private func canRequestReturn(for order: TregoOrderItem) -> Bool {
        tregoReturnableOrderItemId(order) != nil
    }
    ...
    private var filteredOrders: [TregoOrderItem] {
        ...
        return store.orders.filter { tregoOrder($0, matches: filter) }
    }
}
...
private struct TregoReturnRequestComposerScreen: View {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                TregoNoticeCard(
                    title: "Return / Refund order",
                    subtitle: "Shkruaj arsyen pasi porosia te jete For review dhe biznesi do ta shqyrtoje kerkesen."
                )
                ...
                TregoMultilineInputCard(title: "Detajet", text: $details, placeholder: "Shkruaj cfare ndodhi dhe pse deshiron return/refund")
                ...
                Button {
                    ...
                    if isSubmitting {
                        ...
                        Text("Send return / refund request").frame(maxWidth: .infinity)
                    }
                }
                .buttonStyle(TregoPrimaryButtonStyle())
            }
    ...
    private func submit() async {
        ...
        guard let orderItemId = tregoReturnableOrderItemId(order) else {
            statusTone = .error
            statusMessage = "Return / refund mund te kerkohet pasi porosia te jete For review."
            return
        }
...
private struct TregoCheckoutScreen: View {
    private struct TregoCheckoutResultPopup: Identifiable {
        let id = UUID()
        let title: String
        let message: String
        let isSuccess: Bool
    ...
    @State private var checkoutResultPopup: TregoCheckoutResultPopup?
    ...
    var body: some View {
        ZStack {
            ...
            if let popup = checkoutResultPopup {
                Color.black.opacity(0.26)
                    .ignoresSafeArea()
                    .onTapGesture {
                        handleCheckoutPopupDismiss(popup)
                    }

                TregoCheckoutResultCard(
                    title: popup.title,
                    message: popup.message,
                    isSuccess: popup.isSuccess,
                    onClose: { handleCheckoutPopupDismiss(popup) }
                )
                .frame(maxWidth: 360)
                .padding(.horizontal, 24)
                .transition(.scale(scale: 0.94).combined(with: .opacity))
    ...
    private func placeOrder() async {
        ...
        guard response.ok == true else {
            presentCheckoutPopup(
                success: false,
                title: "Order declined",
                message: response.message ?? "Porosia nuk u dergua."
            )
            return
        }
        ...
        presentCheckoutPopup(
            ...
            message: response.message ?? "Pagesa u krye dhe porosia u konfirmua me sukses."
        )
    }
    ...
    private func confirmStripePayment() async {
        ...
        guard response.ok == true else {
            presentCheckoutPopup(
                success: false,
                title: "Order declined",
                message: response.message ?? "Pagesa ende nuk eshte konfirmuar."
            )
            return
        }
        ...
        presentCheckoutPopup(
            success: true,
            title: "Order confirmed",
            message: response.message ?? "Pagesa u konfirmua dhe porosia u krijua me sukses."
        )
    }

    private func presentCheckoutPopup(success: Bool, title: String, message: String) {
        withAnimation(.easeOut(duration: 0.18)) {
            checkoutResultPopup = TregoCheckoutResultPopup(
                title: title,
                message: message,
                isSuccess: success
            )
    ...
    private func handleCheckoutPopupDismiss(_ popup: TregoCheckoutResultPopup) {
        withAnimation(.easeOut(duration: 0.18)) {
            checkoutResultPopup = nil
        }
        if popup.isSuccess {
            dismiss()
        }
...
private struct TregoCheckoutResultCard: View {
    let title: String
    let message: String
    let isSuccess: Bool
    let onClose: () -> Void
    ...
    var body: some View {
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(title)
                        ...
                        .foregroundStyle(TregoNativeTheme.primaryText)
                    Text(isSuccess ? "Pagesa dhe porosia u procesuan." : "Porosia nuk u finalizua.")
        ...
        .shadow(color: Color.black.opacity(colorScheme == .dark ? 0.24 : 0.12), radius: 26, y: 12)
    }

    private var accentColor: Color {
        isSuccess ? Color(red: 0.16, green: 0.72, blue: 0.44) : Color(red: 0.92, green: 0.28, blue: 0.22)
    }
}
...
private struct TregoLaunchAdEditorScreen: View {
        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                TregoNoticeCard(
                    ...
                    subtitle: "Krijo reklame ne qender te ekranit ose reklame me foto per slider-in e home page."
                )
                ...
                TregoSectionHeader(title: "Lloji i reklames")
                Picker("Lloji i reklames", selection: $draft.placement) {
                    ForEach(TregoLaunchAdPlacement.allCases) { placement in
                ...
                TregoProductImageEditorCard(
                    ...
                    subtitle: draft.placement == .startupPopup
                        ? "Kjo foto shfaqet ne popup-in qendror kur perdoruesi hap app-in."
                        : "Kjo foto shfaqet ne slider-in e home page. Ky lloj reklame pranon vetem foto.",
                ...
                if draft.placement == .startupPopup {
                    ...
                    TregoNoticeCard(
                        title: "Slider me foto",
                        subtitle: "Per kete reklame ruhet vetem fotoja. Nuk do te shfaqet tekst ne slider."
                    )
                }
                TregoInputCard(
                    title: "Renditja",
    ...
    private func save() async {
        ...
        guard draft.placement == .homeSliderPhoto || !trimmedTitle.isEmpty else {
            statusTone = .error
...
private enum TregoLaunchAdPlacement: String, CaseIterable, Identifiable {
    case startupPopup
    case homeSliderPhoto
    ...
    var title: String {
        switch self {
        case .startupPopup:
            return "Popup ne qender"
        case .homeSliderPhoto:
            return "Slider me foto"
        }
    }
}
...
private struct TregoTapOutsideKeyboardDismissModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            .background(TregoTapOutsideKeyboardDismissView())
    }
}
...
private struct TregoLoginView: View {
    ...
    @State private var isSubmitting = false
    @State private var isBiometricSubmitting = false
    @State private var failedAttempts = 0
    @State private var useFaceIDNextTime = true
    ...
    var body: some View {
        ScrollView {
            TregoAuthSheetCard {
                ...
                if store.hasSavedBiometricLoginCredentials {
                    Button {
                        Task { await submitBiometricLogin() }
                    } label: {
                        if isBiometricSubmitting {
                            ProgressView()
                                .frame(maxWidth: .infinity)
                        } else {
                            Label("Hyr me Face ID", systemImage: "faceid")
                                .frame(maxWidth: .infinity)
                        }
                    }
                ...
                Toggle(isOn: $useFaceIDNextTime) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Perdor Face ID heren tjeter")
                            .font(.system(size: 14, weight: .semibold))
                        Text("Mos shkruaj cdo here email-in dhe fjalekalimin.")
                            .font(.system(size: 12, weight: .medium))
                            .foregroundStyle(.secondary)
                    }
                }
        ...
        .onAppear {
            ...
            triggerAutomaticBiometricLoginIfNeeded()
        }
        .onChange(of: store.shouldAutoPromptBiometricLogin) { _ in
            triggerAutomaticBiometricLoginIfNeeded()
        }
    }
    ...
    private func submit() async {
        ...
        if let message {
            ...
            failedAttempts = 0
            errorMessage = ""
            store.updateBiometricLoginPreference(
                enabled: useFaceIDNextTime,
                identifier: identifier,
                password: password
            )
            store.shouldAutoPromptBiometricLogin = false
        }
    }

    private func submitBiometricLogin() async {
        guard !isBiometricSubmitting else { return }
        let context = LAContext()
        var authError: NSError?
        let reason = "Perdor Face ID per t'u kycur pa shkruar email dhe fjalekalim."

        guard context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &authError) else {
            errorMessage = "Face ID nuk eshte i disponueshem ne kete pajisje."
            return
        }
        ...
        do {
            let granted = try await context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason)
            guard granted else {
                errorMessage = "Autentikimi me Face ID nuk u krye."
                return
            }

            let message = await store.loginWithSavedBiometricCredentials()
            if let message {
                handleLoginFailure(message)
                store.shouldAutoPromptBiometricLogin = false
            } else {
    ...
    private func triggerAutomaticBiometricLoginIfNeeded() {
        guard store.shouldAutoPromptBiometricLogin else { return }
        guard store.hasSavedBiometricLoginCredentials else {
            store.shouldAutoPromptBiometricLogin = false
            return
        }
        Task { await submitBiometricLogin() }
        ...
        ScrollView {
            TregoAuthSheetCard {
                ...
                Button {
                    ...
                    HStack(spacing: 12) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Me klikim e signup button")
                                .font(.system(size: 14, weight: .semibold))
                                .foregroundStyle(TregoNativeTheme.primaryText)
                            Text("un pranoj termat dhe kushte, dhe poashtu te marr promocione.")
                                .font(.system(size: 12, weight: .medium))
                                .foregroundStyle(TregoNativeTheme.secondaryText)
                                .fixedSize(horizontal: false, vertical: true)
                        }
...
private struct TregoSignupConsentSheet: View {
    @Environment(\.dismiss) private var dismiss
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 16) {
                TregoNoticeCard(
                    title: "Signup consent",
                    subtitle: "Me vazhdimin e regjistrimit, pranon keto pika."
                )

                TregoInfoTile(
                    title: "Pranoj termat dhe kushtet",
                    value: "Duke krijuar llogarine, pranon rregullat dhe kushtet e perdorimit te app-it."
                )

                TregoInfoTile(
                    title: "Pranoj te marr promocione",
                    value: "Mund te marresh oferta, zbritje dhe njoftime promocionale me email."
                )
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 18)
...
private struct TregoEditableAccountTextRow: View {
    let title: String
    @Binding var text: String
...
private struct TregoEditableBirthDateRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text("Perdoret per te plotesuar te dhenat personale")
                    .font(.system(size: 12, weight: .medium, design: .rounded))
                    .foregroundStyle(TregoNativeTheme.secondaryText)
            }
...
private struct TregoEditableGenderRow: View {
    ...
    private var selectedLabel: String {
        options.first(where: { $0.0 == gender })?.1 ?? "Zgjidh gjinine"
    }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Picker("Gjinia", selection: $gender) {
                ...
                Text("Per ta pershtatur me mire profilin tend")
                    .font(.system(size: 12, weight: .medium, design: .rounded))
                    .foregroundStyle(TregoNativeTheme.secondaryText)
            }
...
private struct TregoEditableSecureAccountRow: View {
    let title: String
    @Binding var text: String
    let placeholder: String
    let systemImage: String
...
private struct TregoSettingsPickerAccountRow: View {
    let title: String
    let value: String
    let systemImage: String
    let helper: String
...
private struct TregoActionAccountRow: View {
    let title: String
    let value: String
    let systemImage: String
    let helper: String
...
private struct TregoAuthSocialButtonsGroup: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 14) {
                TregoAuthSocialCircleButton(action: handleAppleTap) {
                    Image(systemName: "apple.logo")
                        .font(.system(size: 20, weight: .bold))
                        .foregroundStyle(colorScheme == .dark ? Color.white : Color.black)
                }

                TregoAuthSocialCircleButton(action: handleGoogleTap) {
                    Text("G")
                        .font(.system(size: 20, weight: .bold))
                        .foregroundStyle(googleBadgeText)
                }

                TregoAuthSocialCircleButton(action: handleFacebookTap) {
                    Text("f")
                        .font(.system(size: 22, weight: .bold))
...
private struct TregoAuthSocialCircleButton<Content: View>: View {
    @Environment(\.colorScheme) private var colorScheme
    let action: () -> Void
    @ViewBuilder let content: () -> Content
    ...
    var body: some View {
        Button(action: action) {
            content()
                ...
                .overlay {
                    Circle()
                        .strokeBorder(colorScheme == .dark ? Color.white.opacity(0.14) : Color.white.opacity(0.52), lineWidth: 0.9)
                }
        }
        .buttonStyle(.plain)
    }
...
private struct TregoLaunchAdManagementRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                TregoMetaPill(text: placementLabel)
    ...
    private var placementLabel: String {
        TregoLaunchAdFormDraft.isSliderDisplayAd(launchAd) ? "Slider me foto" : "Popup ne qender"
    }
}
            ...
            ZStack(alignment: .bottomTrailing) {
                ...
                if user.isEmailVerified == true {
                    Image(systemName: "checkmark.seal.fill")
                        ...
                        .padding(2)
                        .background(TregoNativeTheme.systemBackground, in: Circle())
                        .offset(x: 3, y: 3)
                }
            }
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            Menu {
                ...
                Button("Fjalekalimi", action: onPassword)
                Button("Fshij", role: .destructive, action: onDelete)
            } label: {
                Image(systemName: "plus")
                    .font(.system(size: 13, weight: .bold))
                    .foregroundStyle(TregoNativeTheme.primaryText)
...
private enum TregoAdminUserCategory: String, CaseIterable, Identifiable {
    case admin
    case business
    case client
...
private struct TregoAdminBusinessRow: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            HStack(alignment: .top, spacing: 12) {
                ZStack(alignment: .bottomTrailing) {
                    TregoAvatarView(name: business.businessName ?? business.ownerName ?? "B")
                        .frame(width: 52, height: 52)
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(alignment: .top, spacing: 8) {
                    ...
                    Menu {
                        ...
                        Button("Editim e produktit", action: onEdit)
                        Button(product.isPublic == true ? "Fsheh" : "Publiko", action: onToggleVisibility)
                        Button("Fshije", role: .destructive, action: onDelete)
                    } label: {
                        Image(systemName: "plus")
                            .font(.system(size: 13, weight: .bold))
                            .foregroundStyle(TregoNativeTheme.primaryText)
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 8) {
                if let trackingCode = order.trackingCode, !trackingCode.isEmpty {
                    TregoMetaPill(text: "Tracking \(trackingCode)")
                }
            ...
            if let onReturn {
                Button("Return / Refund", action: onReturn)
                    .buttonStyle(TregoMiniButtonStyle(tint: TregoNativeTheme.accent))
            }
        }
        .padding(16)
...
private struct TregoOrderManagementCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if let eta = order.estimatedDeliveryText, !eta.isEmpty {
                TregoInfoTile(title: "ETA", value: eta)
            }
    ...
    private var statusTint: Color {
        switch (order.fulfillmentStatus ?? order.status ?? "").trimmingCharacters(in: .whitespacesAndNewlines).lowercased() {
        case "confirmed":
            return Color(red: 0.20, green: 0.62, blue: 0.36)
        case "packed":
            return Color(red: 0.98, green: 0.63, blue: 0.14)
        case "shipped":
            return Color(red: 0.24, green: 0.54, blue: 0.98)
        case "delivered":
            return Color(red: 0.16, green: 0.72, blue: 0.44)
        case "returned", "refunded":
            return Color(red: 0.92, green: 0.42, blue: 0.16)
        case "cancelled", "rejected":
            return Color.red.opacity(0.82)
        default:
            return TregoNativeTheme.softAccent
        }
    }
}
...
private struct TregoCartBusinessGroupSkeleton: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                TregoSkeletonBlock(cornerRadius: 12, width: 24, height: 24)
                TregoSkeletonBlock(cornerRadius: 8, width: 120, height: 16)
                Spacer()
                TregoSkeletonBlock(cornerRadius: 14, width: 48, height: 24)
            }

            ForEach(0..<2, id: \.self) { _ in
                HStack(alignment: .top, spacing: 12) {
                    TregoSkeletonBlock(cornerRadius: 12, width: 24, height: 24)
                    TregoSkeletonBlock(cornerRadius: 18, width: 78, height: 78)
                    VStack(alignment: .leading, spacing: 8) {
                        TregoSkeletonBlock(cornerRadius: 8, height: 14)
                        TregoSkeletonBlock(cornerRadius: 8, width: 120, height: 12)
                        TregoSkeletonBlock(cornerRadius: 8, width: 88, height: 14)
                    }
                }
                .padding(12)
                .background(TregoNativeTheme.cardFill, in: RoundedRectangle(cornerRadius: 28, style: .continuous))
...
private struct TregoCartSummarySkeleton: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            TregoSkeletonBlock(cornerRadius: 8, width: 52, height: 12)
            TregoSkeletonBlock(cornerRadius: 10, width: 128, height: 24)
            TregoSkeletonBlock(cornerRadius: 8, height: 12)
            TregoSkeletonBlock(cornerRadius: 8, height: 12)
            TregoSkeletonBlock(cornerRadius: 14, height: 48)
        }
    }
}
...
private struct TregoOrderCardSkeleton: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            TregoSkeletonBlock(cornerRadius: 8, width: 84, height: 12)
            TregoSkeletonBlock(cornerRadius: 10, height: 18)
            TregoSkeletonBlock(cornerRadius: 10, width: 140, height: 18)
            HStack(spacing: 8) {
                TregoSkeletonBlock(cornerRadius: 16, width: 84, height: 28)
                TregoSkeletonBlock(cornerRadius: 16, width: 116, height: 28)
            }
        }
        .padding(18)
        .tregoGlassRectBackground(cornerRadius: 30)
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if isDeleted {
                    ...
                    if let body = visibleBody, !body.isEmpty {
                        Text(body)
                            .font(.system(size: 15, weight: .medium))
                            .foregroundStyle(message.isOwn == true ? TregoNativeTheme.primaryText : Color.primary.opacity(0.92))
                    }
    ...
    private var messageBubbleBackground: some View {
        ...
        if message.isOwn == true {
            bubbleShape
                ...
                .overlay {
                    bubbleShape
                        .strokeBorder(TregoNativeTheme.accent.opacity(0.12), lineWidth: 0.8)
                }
        } else {
            bubbleShape
...
private struct TregoToastView: View {
    ...
    var body: some View {
        ...
        .overlay {
            Capsule()
                .strokeBorder(colorScheme == .dark ? .white.opacity(0.14) : .white.opacity(0.52), lineWidth: 0.8)
        }
...
private struct TregoLaunchAdFormDraft {
    private static let sliderPhotoPlaceholderTitle = "__trego_slider_photo__"

    var placement: TregoLaunchAdPlacement
    var badge: String
    var title: String
    var subtitle: String
    ...
    init(launchAd: TregoLaunchAd?) {
        let defaultStart = Date()
        let defaultEnd = Date().addingTimeInterval(60 * 60 * 24 * 7)
        let existingTitle = launchAd?.title?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""

        placement = Self.isSliderPlaceholderTitle(existingTitle) || existingTitle.isEmpty ? .homeSliderPhoto : .startupPopup
        badge = launchAd?.badge ?? ""
        title = Self.isSliderPlaceholderTitle(existingTitle) ? "" : (launchAd?.title ?? "")
    ...
    func backendPayload(existingId: Int?) -> [String: Any] {
        let trimmedSortOrder = sortOrderText.trimmingCharacters(in: .whitespacesAndNewlines)
        let isPopup = placement == .startupPopup
        var payload: [String: Any] = [
            "badge": isPopup ? badge.trimmingCharacters(in: .whitespacesAndNewlines) : "",
            "title": isPopup ? title.trimmingCharacters(in: .whitespacesAndNewlines) : Self.sliderPhotoPlaceholderTitle,
            "subtitle": isPopup ? subtitle.trimmingCharacters(in: .whitespacesAndNewlines) : "",
            "imagePath": imagePath.trimmingCharacters(in: .whitespacesAndNewlines),
            "ctaLabel": isPopup ? ctaLabel.trimmingCharacters(in: .whitespacesAndNewlines) : "",
            "isActive": isActive,
        ]
        if let sortOrder = Int(trimmedSortOrder), !trimmedSortOrder.isEmpty {
            payload["sortOrder"] = sortOrder
        } else {
            payload["sortOrder"] = 0
        }
        if hasStartDate {
            payload["startsAt"] = TregoNativeFormatting.storageDateString(from: startsAt)
        }
        if hasEndDate {
            payload["endsAt"] = TregoNativeFormatting.storageDateString(from: endsAt)
        }
        if let existingId {
    ...
    static func isSliderPlaceholderTitle(_ title: String?) -> Bool {
        title?.trimmingCharacters(in: .whitespacesAndNewlines) == sliderPhotoPlaceholderTitle
    }

    static func isSliderDisplayAd(_ launchAd: TregoLaunchAd) -> Bool {
        let title = launchAd.title?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return title.isEmpty || isSliderPlaceholderTitle(title)
    }
}
...
private struct TregoOrderStatusDraft {
    ...
    private static func allowedStatuses(from current: String) -> [(String, String)] {
        ...
        case "pending_confirmation":
            return [("confirmed", "Waiting for shipping"), ("cancelled", "Anulo")]
        case "confirmed":
            return [("packed", "Ready to ship"), ("cancelled", "Anulo")]
        case "packed":
            return [("shipped", "Shipped")]
        case "shipped":
            return [("delivered", "For review"), ("returned", "Return / Refund")]
        case "delivered":
            return [("returned", "Return / Refund")]
        default:
            return []
        }
    }
